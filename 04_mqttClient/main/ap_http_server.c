    ...
    while (remaining > 0) 
    {
        ...
        if (pJsonRoot !=NULL)
        {
            ...
            if(pData) {
                ...
                {
                    strcpy(config_wifi.config_wifi_name, pData->valuestring);               // 拷贝内容到字符串数组
                    config_wifi.config_useable = CONFIG_OK;
                    ESP_LOGI(HTTP_TAG, "wifi_name = %s\r\n",config_wifi.config_wifi_name);
                }
            }
            else {
                ESP_LOGI(HTTP_TAG, "error\r\n");
                config_wifi.config_useable = CONFIG_NOK;
                return ESP_FAIL;
            }
            ...
            if(pData) {
                if (cJSON_IsString(pData))                           // 判断字段是否string类型
                {
                    strcpy(config_wifi.config_wifi_password, pData->valuestring);               // 拷贝内容到字符串数组
                    config_wifi.config_useable = CONFIG_OK;
